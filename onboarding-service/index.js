...
// Helper para llamadas HTTP internas entre microservicios
...
// POST /api/onboarding/start — Orquesta el flujo completo de registro
app.post('/api/onboarding/start', async (req, res) => {
  ...
  if (!clientData || !moduleIds || !paymentMethod)
    return res.status(400).json({
      success: false,
      message: 'Se requiere clientData, moduleIds y paymentMethod',
    });
  ...
  try {
    ...
    // ── PASO 2: Cotizar módulos seleccionados ─────────────────────────
    ...
    if (!payRes.ok) {
      ...
      return res.status(402).json({
        success: false, sessionId: session.id, steps,
        message: 'Pago rechazado. Intente con otro método de pago.',
      });
    }
    const payment = payRes.data.data;
    addStep('process_payment', 'completed', `Transacción ${payment.transaction_id} aprobada`);

    // ── PASO 4: Notificar bienvenida (fallo aquí no detiene el proceso)
    try {
    ...
    return res.status(500).json({ success: false, sessionId: session.id, steps, message: 'Error interno del orquestador' });
  }
});

// GET /api/onboarding/:id — Consultar estado de una sesión
...
// GET /api/onboarding — Todas las sesiones (útil para soporte)
app.get('/api/onboarding', async (req, res) => {
  try {
    const result = await db.query('SELECT * FROM onboarding_sessions ORDER BY started_at DESC');
